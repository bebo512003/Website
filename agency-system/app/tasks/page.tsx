...
export default function TasksPage() {
  ...
  return <Page><PageHeader eyebrow="TASKS / BOARD" title="Tasks" description="This board contains tasks from projects your account is authorized to access." action={can('task.create') ? <button className={primaryButtonClassName} onClick={openCreate} disabled={!projects.length}><Plus className="h-4 w-4" />New task</button> : undefined} />{error && <InlineAlert>{error}</InlineAlert>}{loading ? <Panel><LoadingState label="Loading tasks…" /></Panel> : !projects.length ? <Panel><EmptyState icon={CheckSquare} title="No accessible projects" description={can('project.create') ? 'Create a project before adding tasks.' : 'A manager must assign a project to your account.'} /></Panel> : tasks.length === 0 ? <Panel><EmptyState icon={CheckSquare} title="No tasks yet" description="Create the first task for an accessible project." action={can('task.create') ? <button className={primaryButtonClassName} onClick={openCreate}><Plus className="h-4 w-4" />New task</button> : undefined} /></Panel> : <div className="grid gap-4 xl:grid-cols-4">{columns.map((column) => <Panel key={column.id} title={`${column.label} · ${grouped[column.id].length}`}><div className="space-y-3 p-3">{grouped[column.id].length === 0 ? <p className="p-4 text-center text-xs text-text-tertiary">No tasks</p> : grouped[column.id].map((task) => <article key={task.id} className="rounded-md border border-border bg-surface-raised p-4"><div className="flex items-start justify-between gap-2"><Link href={`/projects/${task.project_id}`} className="text-sm font-semibold hover:text-accent">{task.title}</Link>{(can('task.delete') || task.created_by === user?.id) && <button onClick={() => void remove(task)} className="text-text-tertiary hover:text-red-400" aria-label={`Delete ${task.title}`}><Trash2 className="h-3.5 w-3.5" /></button>}</div><p className="mt-2 text-xs text-text-tertiary">{task.projects?.name || 'Project unavailable'}</p><p className="mt-1 text-xs text-text-tertiary">{task.profiles?.full_name || task.profiles?.email || 'Unassigned'} · {task.priority}</p><select className={`${inputClassName} mt-4 py-2 text-xs`} value={task.status} onChange={(event) => void move(task, event.target.value as TaskStatus)} aria-label={`Move ${task.title}`}>{columns.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></article>)}</div></Panel>)}</div>}
