...
export function PortfolioProjectPage() {
  ...
  if (error || !project) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-[#080808] px-5 py-12 text-center text-white">
        <div className="mb-5 flex h-12 w-12 items-center justify-center border border-white/15 bg-white/[0.04]"><ImageIcon className="h-5 w-5 text-accent" /></div>
        <p className="font-mono-tech text-[10px] text-accent">404 / PROJECT NOT FOUND</p>
        <h1 className="mt-4 break-words font-display text-[clamp(3.5rem,16vw,5rem)] leading-none">NOT PUBLIC YET.</h1>
        <p className="mt-4 max-w-md text-sm leading-6 text-white/45">This project may be unpublished, archived, or the link may be incorrect.</p>
        <div className="mt-8 flex w-full max-w-md flex-col gap-3 sm:flex-row sm:justify-center">
          <Link href="/portfolio" className="inline-flex min-h-11 items-center justify-center gap-2 border border-accent bg-accent px-4 py-3 text-sm font-semibold text-accent-foreground"><ArrowLeft className="h-4 w-4" /> Back to portfolio</Link>
          <Link href="/" className="inline-flex min-h-11 items-center justify-center border border-white/20 px-4 py-3 text-sm font-semibold hover:border-accent">Public home</Link>
        </div>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-[#080808] text-white">
      <header className="border-b border-white/10 bg-[#080808]/95 px-4 py-4 backdrop-blur sm:px-8 sm:py-5 lg:px-10">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
          <Link href="/" className="flex min-h-11 items-center gap-3 text-white/70 transition hover:text-white" aria-label="Agency OS home">
            <span className="flex h-9 w-9 items-center justify-center border border-white/20 text-accent"><Layers3 className="h-4 w-4" /></span>
            <span className="hidden text-sm font-bold tracking-[0.22em] sm:inline">AGENCY OS</span>
          </Link>
          <div className="flex items-center gap-2">
            <Link href="/portfolio" className="hidden min-h-11 items-center px-3 text-xs text-white/60 transition hover:text-white sm:inline-flex">Portfolio</Link>
            <Link href="/forms" className="inline-flex min-h-11 items-center gap-2 border border-accent bg-accent px-3 py-2 text-xs font-semibold text-accent-foreground transition hover:brightness-110">Request a Project <ArrowUpRight className="h-3.5 w-3.5" /></Link>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-5 pb-20 pt-8 sm:px-8 lg:px-10 lg:pt-12">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-end lg:gap-20">
          <div><p className="flex items-center gap-3 font-mono-tech text-[10px] text-accent"><span className="h-px w-8 bg-accent" />{project.portfolio_categories?.name || 'Selected work'}</p><h1 className="mt-5 max-w-4xl break-words font-display text-[clamp(3.5rem,16vw,8rem)] leading-[0.82] tracking-tight">{project.title}</h1></div>
        ...
        <div className="mt-12 grid gap-10 border-b border-white/10 pb-12 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20"><div><p className="font-mono-tech text-[10px] text-accent">PROJECT INFORMATION</p><dl className="mt-6 grid grid-cols-1 gap-x-6 gap-y-6 text-sm min-[420px]:grid-cols-2"><div><dt className="text-xs text-white/35">Client / brand</dt><dd className="mt-1 font-medium text-white/80">{project.client_name || 'Confidential'}</dd></div><div><dt className="text-xs text-white/35">Date</dt><dd className="mt-1 font-medium text-white/80">{project.project_date ? new Date(`${project.project_date}T00:00:00`).toLocaleDateString('en-US', { year: 'numeric', month: 'long' }) : 'Not specified'}</dd></div><div className="min-[420px]:col-span-2"><dt className="text-xs text-white/35">Services</dt><dd className="mt-2 flex flex-wrap gap-2">{project.services.length ? project.services.map((service) => <span key={service} className="border border-white/15 px-2.5 py-1 text-xs text-white/65">{service}</span>) : <span className="text-white/50">Creative direction and design</span>}</dd></div></dl></div><div className="max-w-2xl"><p className="text-lg leading-8 text-white/72 sm:text-xl">{project.description || 'Every project is an opportunity to make a clear point of view visible.'}</p></div></div>
